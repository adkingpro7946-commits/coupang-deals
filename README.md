...
**라이브:** https://adkingpro7946-commits.github.io/coupang-deals/
...
검색/딥링크 API는 **시간당 사용 횟수 한도**가 있고, **3회 초과하면 파트너스 이용이 제한**된다. 키워드를 한 번에 너무 많이(약 50개+) 돌리면 한도에 걸린다.

그래서 자동 실행은 **회전 수집**을 쓴다:

```bash
node scripts/fetch-products.mjs --rotate 14
```

`keywords.txt`의 큰 풀에서 매 실행 14개씩만 순환 사용하고, **기존 `products.json`과 병합**한다. 호출을 한도 아래로 유지하면서 며칠에 걸쳐 카탈로그를 넓게 쌓는다. 회전 위치는 `data/rotation.json`에, 상품별 마지막 관측 시각은 `lastSeen`에 저장되며, 14일 넘게 안 보인 상품은 정리된다(최대 800개).
